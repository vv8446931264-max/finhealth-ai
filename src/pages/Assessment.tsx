...
export default function AssessmentPage({ navigate }: { navigate: (p: string) => void }) {
  ...
  const next = () => {
    ...
    else { saveAssessment(data); navigate("/dashboard"); }
  };
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10 md:py-16">
        <div className="mb-8">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {STEPS.map((label, i) => (
              <div key={label} className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition ${
                i < step ? "border-emerald-500/40 bg-emerald-50 text-emerald-700"
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-elegant md:p-10">
          ...
          <div className="mt-8 space-y-6">
            {step === 0 && (<>
              ...
              <MoneyField label="Monthly discretionary spending (dining, entertainment)" value={data.discretionary} onChange={(v) => update("discretionary", v)} placeholder="e.g. 12000" />
            </>)}
            {step === 1 && (<>
            ...
            {step === 2 && (<>
              ...
              <SliderField label="Existing credit score" min={300} max={900} step={10} value={data.creditScore} onChange={(v) => update("creditScore", v)} suffix="" />
            </>)}
            {step === 3 && (<>
              <SliderField label="Emergency fund (in months of expenses)" min={0} max={12} step={1} value={data.emergencyFundMonths} onChange={(v) => update("emergencyFundMonths", v)} suffix=" months" />
              ...
              <div>
                ...
                <Input type="number" min={0} value={data.dependents} onChange={(e) => update("dependents", Math.max(0, parseInt(e.target.value || "0")))} className="mt-2 h-11" />
              </div>
            </>)}
          </div>
          ...
          <div className="mt-10 flex items-center justify-between">
            ...
            <Button onClick={next} className="gap-2 bg-navy text-white hover:bg-navy/90" disabled={step === 0 && data.income <= 0}>
              {step === STEPS.length - 1 ? "See My Score" : "Continue"} <ArrowRight className="h-4 w-4" />
            </Button>
          </div>
        </div>
      </main>
...
function MoneyField({ label, value, onChange, placeholder }: { label: string; value: number; onChange: (v: number) => void; placeholder?: string }) {
  return (
    <div>
      <Label className="text-sm font-medium">{label}</Label>
      <div className="relative mt-2">
        <IndianRupee className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input type="number" min={0} value={value || ""} placeholder={placeholder} onChange={(e) => onChange(Math.max(0, parseFloat(e.target.value || "0")))} className="h-11 pl-9" />
      </div>
    </div>
  );
}
...
function SliderField({ label, min, max, step, value, onChange, suffix }: { label: string; min: number; max: number; step: number; value: number; onChange: (v: number) => void; suffix?: string }) {
  return (
    <div>
      ...
      <Slider min={min} max={max} step={step} value={[value]} onValueChange={([v]) => onChange(v)} className="mt-4" />
